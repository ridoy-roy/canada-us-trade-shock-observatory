...
def panel_chart(
    ...
    for panel_index, (field, label, color, multiplier) in enumerate(specs):
        ...
        drawing.add(String(left, panel_top + 7, label, fontName="Helvetica-Bold", fontSize=8.5, fillColor=NAVY))
        for tick in range(3):
            ...
            drawing.add(String(left - 5, y - 2.5, f"{value:,.0f}", textAnchor="end", fontSize=6.5, fillColor=SLATE))
        ...
        if panel_index == len(specs) - 1:
            ...
            for i, row in enumerate(rows):
                if i % step == 0 or i == len(rows) - 1:
                    drawing.add(String(xs[i], panel_bottom - 12, row["month"], textAnchor="middle", fontSize=5.8, fillColor=SLATE))
    return drawing


def product_decline_chart(rows: list[dict[str, str]], width: float = 510, height: float = 292) -> Drawing:
    left, right, top, bottom = 174, 55, 15, 25
    drawing = Drawing(width, height)
    plot_width = width - left - right
    ...
    for tick in range(5):
        ...
        drawing.add(String(x, 7, f"${maximum * tick / 4 / 1_000_000:.0f}M", textAnchor="middle", fontSize=6.5, fillColor=SLATE))
    for index, (row, value) in enumerate(zip(rows, values)):
        y = height - top - (index + 1) * row_height
        bar_width = plot_width * value / maximum
        label = f'{row["hs6"]}  {row["product_name"]}'
        drawing.add(String(left - 6, y + row_height * .37, label, textAnchor="end", fontName="Helvetica-Bold", fontSize=5.8, fillColor=NAVY))
        drawing.add(Rect(left, y + row_height * .12, bar_width, row_height * .5, rx=2, ry=2, fillColor=BLUE, strokeColor=None))
        drawing.add(String(left + bar_width + 5, y + row_height * .37, f"-${value / 1_000_000:.1f}M", fontSize=7, fillColor=SLATE))
    return drawing
...
def build() -> Path:
    ...
    styles.add(ParagraphStyle(name="CoverTitle", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=28, leading=33, textColor=colors.white, alignment=TA_LEFT, spaceAfter=16))
    styles.add(ParagraphStyle(name="CoverSub", parent=styles["Normal"], fontSize=12.5, leading=18, textColor=colors.HexColor("#DCE7FA"), spaceAfter=10))
    styles.add(ParagraphStyle(name="H1x", parent=styles["Heading1"], fontName="Helvetica-Bold", fontSize=19, leading=23, textColor=NAVY, spaceBefore=5, spaceAfter=10))
    ...
    styles.add(ParagraphStyle(name="Bodyx", parent=styles["BodyText"], fontSize=9.2, leading=13.2, textColor=colors.HexColor("#253047"), spaceAfter=7))
    styles.add(ParagraphStyle(name="Smallx", parent=styles["BodyText"], fontSize=7.5, leading=10.2, textColor=SLATE, spaceAfter=4))
    styles.add(ParagraphStyle(name="Callout", parent=styles["BodyText"], fontName="Helvetica-Bold", fontSize=11, leading=15, textColor=NAVY, borderColor=BLUE, borderWidth=0, borderPadding=8, backColor=colors.HexColor("#EFF6FF"), spaceBefore=4, spaceAfter=10))
    ...
    cards = [
        ...
        [Paragraph(usd(annual_2025["value"]), styles["H1x"]), Paragraph(f"{annual_2025['value'] / annual_2024['value'] - 1:+.1%}", styles["H1x"]), Paragraph(usd(annual_2025["duty"]), styles["H1x"])],
    ]
    card_table = Table(cards, colWidths=[2.35 * inch] * 3, rowHeights=[0.32 * inch, 0.58 * inch])
    card_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), LIGHT), ("BOX", (0, 0), (-1, -1), 0.5, GRID), ("INNERGRID", (0, 0), (-1, -1), 0.5, colors.white), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("LEFTPADDING", (0, 0), (-1, -1), 10)]))
    story += [card_table, Spacer(1, 16), Paragraph("Main result", styles["H2x"]), Paragraph("The value of Canadian core-steel imports in scope was 36.8% lower in 2025 than in 2024. Year-over-year declines were larger in the later tariff periods, although the comparison by itself cannot establish that the tariff caused the change.", styles["Callout"]), Paragraph(f"Release date: {date.today().isoformat()} | Validation: {validation['status'].upper()} | {validation['raw_snapshot_count']} archived API responses", styles["Smallx"]), PageBreak()]
    ...
    for row in pilot:
        pilot_table.append([row["month"], usd(float(row["import_value_usd"])), f"{int(row['quantity_1']):,}", usd(float(row["calculated_duty_usd"])), row["policy_regime"].replace("_", " ")])
    ...
    qt.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), NAVY), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("GRID", (0, 0), (-1, -1), 0.4, GRID), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, LIGHT]), ("FONTSIZE", (0, 0), (-1, -1), 8), ("LEFTPADDING", (0, 0), (-1, -1), 6), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5)]))
    story += [qt, Paragraph("Limitations", styles["H2x"]), Paragraph("This release covers one trade flow over two years. It does not measure welfare, tariff pass-through, trade diversion, domestic output, employment, or a causal treatment effect. Product coverage follows the original core-steel ranges at HS6; later derivative additions are outside the initial scope.", styles["Bodyx"]), Paragraph("Reproduction", styles["H2x"]), Paragraph("Run <font name='Courier'>python scripts/build_release.py --reuse-snapshots</font> to rebuild the processed files from the archived source responses. Run <font name='Courier'>python -m unittest discover -s tests -v</font> with the project source path configured to repeat the validation tests.", styles["Bodyx"]), Paragraph("Official sources", styles["H2x"])]
    sources = [
